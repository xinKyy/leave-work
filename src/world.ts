...
export class OfficeWorld {
  ...
  private buildOffice() {
    ...
    this.cardMesh = new THREE.Mesh(new THREE.BoxGeometry(.45, .055, .3), new THREE.MeshStandardMaterial({ color: 0xe2ad45, emissive: 0x4a2d08, emissiveIntensity: .3 })); this.cardMesh.position.set(KEYCARD.x, .16, KEYCARD.z); this.cardMesh.rotation.y = -.3; this.cardMesh.castShadow = true; this.office.add(this.cardMesh); this.cardLabel = makeLabel('门禁卡', '#b78335', 20); this.cardLabel.position.set(KEYCARD.x, .62, KEYCARD.z); this.cardLabel.rotation.x = -Math.PI / 2; this.cardLabel.scale.set(1.25, .32, 1); this.office.add(this.cardLabel); this.cardBeacon = new THREE.Mesh(new THREE.RingGeometry(.62, .74, 32), new THREE.MeshBasicMaterial({ color: COLORS.amber, transparent: true, opacity: .8, side: THREE.DoubleSide, depthWrite: false })); this.cardBeacon.rotation.x = -Math.PI / 2; this.cardBeacon.position.set(KEYCARD.x, .07, KEYCARD.z); this.cardBeacon.renderOrder = 10; this.office.add(this.cardBeacon);
